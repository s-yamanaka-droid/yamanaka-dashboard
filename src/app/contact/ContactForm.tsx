...
export function ContactForm({initialTopic = "ai-consult",projectName}:{initialTopic?:string;projectName?:string}) {
 const [topic,setTopic]=useState(topics.some(([id])=>id===initialTopic)?initialTopic:"ai-consult");
 const [status,setStatus]=useState("");
 return <section id="inquiry" className="contact-layout"><aside className="contact-aside"><p className="eyebrow">CONTACT / LAKKAN</p><h2><span>まずは、</span><span>話そう。</span></h2><p>人のこと。仕事の仕組みのこと。<br/>いま困っていることから、お聞かせください。</p><div className="contact-guide"><span>01 — テーマを選ぶ</span><span>02 — 相談内容を書く</span><span>03 — メールで送る</span></div></aside><form className="contact-form" onSubmit={e=>{e.preventDefault();const data=new FormData(e.currentTarget);const body=["お名前："+data.get("name"),"メール："+data.get("email"),"会社名："+data.get("company"),"ご相談："+topics.find(([id])=>id===topic)?.[1],"",String(data.get("message"))].join("\n");window.location.href="mailto:s-yamanaka@tre-pro.co.jp?subject="+encodeURIComponent("[Lakkan] "+topics.find(([id])=>id===topic)?.[1])+"&body="+encodeURIComponent(body);setStatus("メールアプリで内容をご確認のうえ、送信してください。");}}>
 ...
 <div><label htmlFor="message">ご相談内容（必須）</label><textarea id="message" name="message" rows={7} required maxLength={4000} defaultValue={projectName?`${projectName}の実績を見て相談したいです。\n\n`:undefined} placeholder="いまの課題や、実現したいことをお聞かせください。"/></div>
