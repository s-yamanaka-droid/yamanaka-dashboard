...
const bgVar = "#070b12"
const fgVar = "#f4f8fc"
const fgMutedVar = (a: number) => `rgba(244,248,252,${a})`
const cardVar = (a = 1) => `rgba(15,22,34,${a})`
const ROW_HEIGHT = 60
...
export default function MusicHero({
  ...
  const reducedMotion = useSyncExternalStore(
    (notify) => { const mq = window.matchMedia("(prefers-reduced-motion: reduce)"); mq.addEventListener("change", notify); return () => mq.removeEventListener("change", notify) },
  ...
  useEffect(() => {
    ...
    announceTimerRef.current = setTimeout(() => {
      ...
      if (t) setAnnouncement(`${t.title}：${t.artist}`)
    }, 400)
    return () => {
      if (announceTimerRef.current) clearTimeout(announceTimerRef.current)
    }
  ...
  useEffect(() => {
    if (reducedMotion) { const id = requestAnimationFrame(() => setIsPlaying(false)); return () => cancelAnimationFrame(id) }
  }, [reducedMotion])
  ...
  const onCardMove = useCallback((e: React.PointerEvent<HTMLDivElement>) => {
    if (reducedMotion || isCoarsePointer || theme === "minimal") return
  ...
  }, [reducedMotion, isCoarsePointer, theme, effectiveFullscreen])

  const onCardLeave = useCallback(() => {
    if (reducedMotion || theme === "minimal") return
    if (effectiveFullscreen) {
      const el = videoWrapRef.current
      if (el) {
  ...
  }, [reducedMotion, theme, effectiveFullscreen, BASE_ROTATE_Y])
  ...
  function togglePlay() {
    setAnnouncement(isPlaying ? "動画を一時停止" : "動画を再生")
    setIsPlaying((p) => !p)
  }
  ...
  if (isCoarsePointer) {
    return (
      <div
        ...
        tabIndex={0}
        role="group"
        aria-label={`事業紹介。${activeTrack.title}。矢印キーで切替、スペースキーで動画を再生・停止。`}
        style={{
          position: "relative",
          inset: 0,
  ...
  return (
    ...
    >
      <style>{`
        ...
        .mh-focusable * { box-sizing: border-box; }
        @media (prefers-reduced-motion: reduce) { .lakkan-video-hero *, .lakkan-video-hero *::before { animation: none !important; transition: none !important; } }
      ...
      <div
        style={{
          position: effectiveFullscreen ? "static" : "relative",
          zIndex: 2,
          display: "flex",
          flexDirection: "column",
          alignItems: "center",
      ...
      >
        ...
        <div style={{ position: effectiveFullscreen ? "static" : "relative" }}>
          <div
            aria-hidden="true"
            style={{
              position: "absolute",
          ...
          <div
            ...
            style={{
              ...
              width: effectiveFullscreen ? "100%" : "min(58dvh, 460px)",
              height: effectiveFullscreen ? "100%" : "min(58dvh, 460px)",
...
function SeamlessLoopVideo({
  ...
  useEffect(() => {
    ...
    if (!active) return
    if (playing) { active.play().catch(() => {}); if (crossfadingRef.current) { const other = activeRef.current === "a" ? bRef.current : aRef.current; other?.play().catch(() => {}) } }
    else { aRef.current?.pause(); bRef.current?.pause() }
  }, [playing])
  ...
  return (
    <>
      <video ref={aRef} src={src} playsInline preload="metadata" muted={muted} style={{ ...base, ...style, opacity: aOpacity }} />
      <video ref={bRef} src={src} playsInline preload="metadata" muted={muted} style={{ ...base, ...style, opacity: bOpacity }} />
    </>
  )
}
...
function PlayerControls({
  ...
  return (
    ...
    >
      ...
      {track.href && <a href={track.href} style={{ display: "inline-block", color: "#fff", fontSize: 12, marginTop: 8, textUnderlineOffset: 4 }}>詳しく見る ↗</a>}
