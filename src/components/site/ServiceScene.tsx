...
type Article = {title:string;category:string;image:string;link:string;lede?:string};
const steps = [
...
export function ServiceScene({id,articles}:{id:string;articles:Article[]}) {
 ...
 if(id==="luna") return <a className="service-scene luna-scene" href="https://luna-tech-public-site.vercel.app/" aria-label="LunaTechの公式サイトを見る"><div className="luna-scene-caption"><span>A mind of your own.</span><ArrowUpRight size={30}/></div><div className="luna-screenshot"><Image src="/works/lunatech-current.jpg" width={1280} height={720} alt="LunaTechの現在の公式サイト" sizes="(max-width: 700px) 90vw, 80vw" /></div><span className="scene-footnote">LUNA AI — BY LUNATECH</span></a>;
 ...
 if(id==="news") return <NewsEditorial articles={articles}/>;
